...
with open(r'C:\\Users\\k3553\\GitHub\\RUD_Project\\rud_stockapi\\kisdev.yaml', encoding='UTF-8') as f:
...
def _setTRENV(cfg):
    nt1 = namedtuple('KISEnv', [
                     'my_app', 'my_sec', 'my_acct', 'my_phone', 'my_prod', 'my_token', 'my_url'])
    d = {
        'my_app': cfg['my_app'],
        'my_sec': cfg['my_sec'],
        'my_acct': cfg['my_acct'],
...
def do_sell(stock_code, order_qty, order_price, prd_code="01", order_type="00"):
    t1 = do_order(stock_code, order_qty, order_price,
                  buy_flag=False, order_type=order_type)
    return t1.isOK()
...
def do_buy(stock_code, order_qty, order_price, prd_code="01", order_type="00"):
    t1 = do_order(stock_code, order_qty, order_price,
                  buy_flag=True, order_type=order_type)
    return t1.isOK()
...
def get_stock_history_by_ohlcv(stock_no, gb_cd='D', adVar=False):
    ...
    chosend_fld = ['stck_bsop_date', 'stck_oprc',
...
def get_stock_investor(stock_no):
    ...
    if t1.isOK():
        ...
        chosend_fld = ['stck_bsop_date', 'prsn_ntby_qty',
        ...
        hdf1[['PerBuy', 'ForBuy', 'OrgBuy']] = hdf1[[
            'PerBuy', 'ForBuy', 'OrgBuy']].apply(pd.to_numeric)
        hdf1['EtcBuy'] = (hdf1['PerBuy'] + hdf1['ForBuy'] +
                          hdf1['OrgBuy']) * -1
        hdf1.set_index('Date', inplace=True)
        # sum을 맨 마지막에 추가하는 경우
