...
public class RudService {
    ...
    public Double getTotlaByUserIdAndDate(String userId, String date) {
        ...
        for (Rud rud : rudList) {
            ...
            dateTotal += (!rud.getPaul() ? rud.getMarketOrder() * rud.getNos() : rud.getMarketOrder() * exchangeRate * rud.getNos());
        }

        // 원화 + 달러 총 자산 계산
        dateTotal += (wallet.getDollar() != null ? wallet.getDollar() * walletService.getExchangeForDate(wallet, date) : 0) + (wallet.getWon() != null ? wallet.getWon() : 0);
        return dateTotal;
    }
    ...
    public List<Log> getLogByUserId(String userId) {
        ...
        for (Rud rud : rudList) {
            ...
            // 국장이면 그대로 해외장이면 환율 곱해줌
            logResponse.addStockValue(!rud.getPaul() ? rud.getMarketOrder() * rud.getNos() : rud.getMarketOrder() * exchangeRate * rud.getNos());
