...
const initialData = [
  ...
  { id: 4, name: 'TSLY', price: 34.50, quantity: 15, targetRatio: 70 },
  { id: 5, name: 'CURE', price: 15.00, quantity: 12, targetRatio: 60 },
];
...
const DataRow = ({ item, totalAssets, totalTargetRatio, onQuantityChange, onTargetRatioChange }) => {
  ...
  return (
    <tr key={item.id}>
      ...
      <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>{balance.toFixed(2)}</td>
      <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>{ratio} %</td>
      <td style={{ width: '9%' }}>
        <input 
          type="number" 
      ...
      <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>{rebalancingRatio} %</td>
      <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>{targetInvestment.toFixed(2)}</td>
      <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>{targetQuantity.toFixed(2)}</td>
      <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>{quantityAdjustment.toFixed(2)}</td>
    </tr>
  );
};
          ...
          <Col md="12">
            <div className="table-responsive">
              <Table className="custom-table">
                <thead>
                  <tr>
                    <th rowSpan="2" style={{ textAlign: 'center', verticalAlign: 'middle' }}>종목명</th>
                    <th rowSpan="2" style={{ textAlign: 'center', verticalAlign: 'middle' }}>주가</th>
                    <th colSpan="3" style={{ textAlign: 'center' }}>현재</th>
                    <th rowSpan="2" style={{ textAlign: 'center', verticalAlign: 'middle' }}>희망비중</th>
                    <th colSpan="3" style={{ textAlign: 'center' }}>리밸런싱</th>
                    <th rowSpan="2" style={{ textAlign: 'center', verticalAlign: 'middle' }}>수량조절</th>
                  </tr>
                  <tr>
                    <th>수량</th>
                    <th>잔고</th>
