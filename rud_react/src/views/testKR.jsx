...
const initialDomesticData = [
    ...
    { id: 7, name: "TREX", price: 290.00, quantity: 12, targetRatio: 60 },
    { id: 8, name: "TREX", price: 290.00, quantity: 12, targetRatio: 60 },
    { id: 9, name: "TREX", price: 290.00, quantity: 12, targetRatio: 60 },
    { id: 10, name: "TREX", price: 290.00, quantity: 12, targetRatio: 60 },
    { id: 11, name: "TREX", price: 290.00, quantity: 12, targetRatio: 60 },
    { id: 12, name: "김세현", price: 290.00, quantity: 12, targetRatio: 60 },
];
...
const TableKR = () => {
    ...
    const handleQuantityChange = (id, newQuantity) => {
        setDomesticData((prevData) => prevData.map((item) => item.id === id ? { ...item, quantity: newQuantity } : item));
    };

    const handleTargetRatioChange = (id, newRatio) => {
        setDomesticData((prevData) => prevData.map((item) => item.id === id ? { ...item, targetRatio: newRatio } : item));
    };

    const totalAssets = domesticData.reduce((total, item) => total + item.price * item.quantity, 0);
    const totalTargetRatio = domesticData.reduce((total, item) => total + item.targetRatio, 0);
                ...
                <Table className="custom-table">
                    <thead>
                        <tr>
                            ...
                            <th rowSpan="2">주가</th>
                            <th colSpan="3">현재</th>
                            <th rowSpan="2">희망비중</th>
                            <th colSpan="3">리밸런싱</th>
                            <th rowSpan="2">수량조절</th>
                        </tr>
                        <tr>
                            <th>수량</th>
                            <th>잔고 (₩)</th>
